export interface AuditInput {
  html: string
  url: string
  metrics?: { lcp?: number; cls?: number; inp?: number; ttfb?: number }
}
...
export class AuditEngine {
  ...
  private report: AuditReport = { seo: [], performance: [], security: [], technical: [] }
  ...
  async auditMultiplePages(pages: Array<{ url: string; html: string }>): Promise<AuditReport> {
    ...
    const report: AuditReport = { seo: [], performance: [], security: [], technical: [] }
  ...
  async auditRobotsTxt(baseUrl: string): Promise<TechnicalIssue[]> {
    const robotsUrl = new URL('/robots.txt', baseUrl).toString()
    const issues: TechnicalIssue[] = []
    if (!this.fetcher) return issues
  ...
  async auditSitemap(baseUrl: string): Promise<TechnicalIssue[]> {
  ...
  async auditLinks(links: string[]): Promise<TechnicalIssue[]> {
    const issues: TechnicalIssue[] = []
    if (!this.fetcher) return issues
  ...
  async auditRedirectChain(redirects: Array<{ from: string; to: string; status: number }>): Promise<TechnicalIssue[]> {
    const issues: TechnicalIssue[] = []
    // Build chain map
  ...
  exportReport(_options: { format: 'json' | 'html' } = { format: 'json' }): string {
