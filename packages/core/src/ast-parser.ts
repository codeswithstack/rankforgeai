  ...
  analyze(code: string, context: AnalyzeContext = {}): ASTIssue[] {
    ...
    lines.forEach((line, idx) => {
      ...
      if (/<Image\s[^>]*src/.test(line) && this.options.framework === 'nextjs') {
        ...
        if (
          context.aboveFoldComponents?.includes(componentName) &&
...
function isInUseEffect(code: string, lineIndex: number): boolean {
  const lines = code.split('\n')
  let depth = 0
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inUseEffect) {
      ...
      if (depth <= 0 && i > 0) inUseEffect = false
    }
  }
  return false
}

export function extractReactMetaTags(code: string): MetaTagResult | null {
...
function extractStringValue(code: string, key: string): string | undefined {
...
export function detectMissingImageProps(code: string): ImagePropIssue[] {
  const issues: ImagePropIssue[] = []
  const lines = code.split('\n')

  lines.forEach((line, idx) => {
    if (!/<img\s/i.test(line)) return
    const lineNum = idx + 1
