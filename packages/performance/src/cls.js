export function detectCLSIssues(page) {
    const issues = [];
...
export function generateCLSFixes(issue) {
    ...
    if (issue.type === 'font-swap-shift') {
        const src = issue.element.src || '/fonts/font.woff2';
        return {
...
export function calculateCLSScore(cls) {
    if (cls === 0)
        return 100;
    if (cls < 0.1)
        return Math.round(100 - (cls / 0.1) * 5);
    if (cls < 0.25)
        return Math.round(95 - ((cls - 0.1) / 0.15) * 45);
