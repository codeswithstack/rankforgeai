...
export function detectTTFBIssues(profile) {
    const issues = [];
...
export function generateTTFBFixes(issue) {
...
export function calculateTTFBScore(ms) {
    if (ms <= 100)
        return 100;
    if (ms <= 300)
        return Math.round(100 - ((ms - 100) / 200) * 10);
    if (ms <= 800)
        return Math.round(90 - ((ms - 300) / 500) * 40);
    if (ms <= 1800)
        return Math.round(50 - ((ms - 800) / 1000) * 50);
    return 0;
}
//# sourceMappingURL=ttfb.js.map