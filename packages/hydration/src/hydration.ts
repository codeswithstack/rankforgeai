...
function parseNodes(html: string): SimpleNode[] {
...
export function detectHydrationMismatches(serverHTML: string, clientHTML: string): HydrationResult {
...
export function suggestUseEffect(originalCode: string, variableName: string): string {
...
export function suggestDynamicImport(componentName: string, importPath: string): string {
